...
export const state: {
  ...
  audio: Buffer;
} = { project: null, rows: {}, audio: Buffer.alloc(0) };
...
export const db: any = {
  ...
  update: (table: string) => ({
    set: (values: any) => ({
      where: async () => {
        for (const row of state.rows[table] ?? []) Object.assign(row, values);
      },
    }),
  }),
  select: () => ({
    from: (table: string) => ({
      where: () =>
        Object.assign(Promise.resolve(state.rows[table] ?? []), {
          limit: async () => state.rows[table] ?? [],
        }),
    }),
  }),
};
...
export function Router() {
  return Object.fromEntries(
    ["use", "get", "post", "patch"].map((method) => [
      method,
      (path: string, handler: (...args: any[]) => any) => {
...
export class ObjectStorageService {
  async getObjectEntityFile() {
    return { download: async () => [state.audio], delete: async () => {} };
  }
  async savePrivateArtifact(audio: Buffer) {
    state.audio = audio;
    return "/objects/rendered-edit";
  }
}
export const sdk: {
  configuration: any;
...
} = { configuration: null, calls: [], generate: async () => ({}) };
export const Type = {
  OBJECT: "OBJECT",
  ARRAY: "ARRAY",
  STRING: "STRING",
