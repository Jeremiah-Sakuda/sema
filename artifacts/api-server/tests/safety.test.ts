...
const configuration = {
  ...
  GOOGLE_CLOUD_SERVICE_ACCOUNT_JSON: JSON.stringify({
    ...
    client_email: "test@sema-test-project.iam.gserviceaccount.com",
...
const beat = {
  ...
  description: "A woman opens the door.",
  importance: "high",
...
test("edited renders are auditionable, revoke approval and never approve implicitly", async () => {
  const render = routes.find(
    (route) => route.method === "post" && route.path.endsWith("/render"),
  )!.handler;
  const decide = routes.find(
    (route) => route.method === "patch" && route.path.endsWith("/decision"),
  )!.handler;
  ...
  globalThis.fetch = async () => {
    ...
    return new Response(
      JSON.stringify({
        audioContent: encodeWave(Buffer.alloc(48000)).toString("base64"),
      }),
    );
  };
  const request = {
    params: { projectId: "project-1", beatId: "beat-1" },
    body: { candidateId: "candidate-1", text: "The door opens." },
    user: { id: "owner" },
    isAuthenticated: () => true,
  };
  const invoke = async (handler: any, req: any) => {
    const result: any = { code: 200, payload: null };
    await handler(
      req,
      {
        headersSent: false,
  ...
  const reset = (end = 3) => {
    state.project = {
      ownerId: "owner",
      status: "needs_review",
      mediaObjectPath: "/objects/source",
      duration: 5,
    };
    state.rows = {
      projects: [state.project],
      beats: [{ beatId: "beat-1", projectId: "project-1", evidenceFrames: [] }],
      candidates: [
        {
  ...
  try {
    reset();
    assert.equal(
      (await invoke(render, { ...request, user: { id: "other" } })).code,
      403,
    );
    assert.equal(
      (
        await invoke(render, {
          ...request,
          body: { ...request.body, candidateId: "different" },
        })
      ).code,
      404,
    );
    assert.equal(syntheses, 0);
    const passed = await invoke(render, request);
    assert.equal(passed.code, 200);
    assert.equal(passed.payload.fitStatus, "pass");
    assert.equal(passed.payload.ttsDuration, 1);
    assert.equal(state.rows.decisions[0].exportAuthorized, false);
    assert.equal(state.rows.beats[0].state, "needs_human");
    await assert.rejects(verifiedExport("project-1"), /authorization/);
    const before = syntheses;
    const stale = await invoke(decide, {
      ...request,
      body: {
        candidateId: "candidate-1",
        action: "human_edited_approve",
        finalText: "An unrendered edit.",
      },
    });
    ...
    assert.equal(
      syntheses,
      before,
      "approval must never synthesize changed wording",
    );
    reset(1.5);
    const failed = await invoke(render, request);
    assert.equal(failed.code, 200);
    assert.equal(failed.payload.fitStatus, "failed");
    assert.equal(
      parseWave(state.audio).duration,
      1,
      "failed fit still has measured audio for audition",
    );
    assert.equal(state.rows.decisions[0].exportAuthorized, false);
    const approval = await invoke(decide, {
      ...request,
      body: {
        candidateId: "candidate-1",
        action: "approve",
        finalText: request.body.text,
      },
    });
...
test("project route guard rejects anonymous decisions and exports and another owner", async () => {
  const guard = routes.find(
    (route) => route.method === "use" && route.path === "/projects/:projectId",
  )!.handler;
  state.project = { ownerId: "owner" };
  for (const request of [
    {
      method: "PATCH",
      path: "/beats/beat-1/decision",
      user: undefined,
      expected: 401,
    },
    ...
    {
      method: "GET",
      path: "/export/sema-narration.wav",
      user: undefined,
      expected: 401,
    },
    {
  ...
  for (const foreign of ["candidate", "window"]) {
    state.rows = {
      ...
      candidates: [
        {
          candidateId: "candidate-1",
          projectId: foreign === "candidate" ? "other-project" : "project-1",
          windowId: "window-1",
        },
      ],
      windows: [
        {
          windowId: "window-1",
          projectId: foreign === "window" ? "other-project" : "project-1",
        },
      ],
    };
    let status = 0;
    ...
    await handler(
      {
        params: { projectId: "project-1", beatId: "beat-1" },
        body: { candidateId: "candidate-1", action: "approve" },
        user: { id: "owner" },
        isAuthenticated: () => true,
      },
      response,
...
test("export binds the exact approved candidate, wording, audio and protected placement", async () => {
  function reset() {
    ...
    state.rows = {
      ...
      windows: [
        { windowId: "window-1", start: 1, end: 3, protectedCollision: false },
      ],
      candidates: [
        {
          candidateId: "candidate-1",
      ...
      processing: [
        {
          manifest: {
            ...
            renders: {
              "candidate-1": {
                text: "A door opens.",
                objectPath: "/objects/audio",
                sha256: audioHash(state.audio),
              },
            },
          },
        },
  ...
  const mutations = [
    ...
    () => {
      state.rows.processing[0].manifest.protectedIntervals = [
        { start: 2, end: 4 },
      ];
    },
    () => {
  ...
  for (const mutate of mutations) {
    reset();
    mutate();
    await assert.rejects(verifiedExport("project"));
  }
  reset();
  state.rows.decisions[0].action = "left_undescribed";
  const omitted = await verifiedExport("project");
  assert.equal(omitted.entries.length, 0);
  assert.equal(
    omitted.decisions.length,
    1,
    "intentional omissions remain in the ledger",
  );
});
...
test("configuration requires the three secret values and strips untrusted auth URLs", () => {
  ...
  assert.deepEqual(
    Object.keys(googleConfiguration(configuration).credentials).sort(),
...
test("sample measurement and stem placement preserve audio and silence", () => {
  ...
  assert.throws(
    () => parseWave(audio.subarray(0, audio.length - 2)),
    /Invalid/,
  );
...
test("fit validation fails closed for collision, invalid measurement and overlong narration", () => {
  ...
  assert.equal(
    verifyRenderedFit({ window, renderedDuration: 1.8 }).status,
    "pass",
  );
  assert.equal(
    verifyRenderedFit({ window, renderedDuration: 1.81 }).status,
    "failed",
  );
  assert.equal(
    verifyRenderedFit({ window, renderedDuration: 1, protectedCollision: true })
      .status,
    "blocked",
  );
  for (const renderedDuration of [0, -1, Infinity, NaN])
    assert.equal(
      verifyRenderedFit({ window, renderedDuration }).status,
      "failed",
    );
...
test("Google calls use Vertex, require timestamps and measure returned synthesis", async () => {
  ...
  try {
    ...
    assert.equal(
      sdk.configuration.googleAuthOptions.credentials.token_uri,
      undefined,
    );
    response = {
      results: [
        {
          alternatives: [
            {
              transcript: "Hello",
              words: [{ startTime: "0.1s", endTime: "0.5s" }],
            },
          ],
        },
      ],
    ...
    assert.deepEqual(
      (await provider.transcribe(Buffer.alloc(32000), 10)).intervals,
      [{ start: 10.1, end: 10.5 }],
    );
    response = { results: [{ alternatives: [{ transcript: "Hello" }] }] };
    await assert.rejects(
      provider.transcribe(Buffer.alloc(32000), 0),
      /without timestamps/,
    );
    response = {
      candidates: [
        {
          finishReason: "STOP",
          content: { parts: [{ text: JSON.stringify({ beats: [beat] }) }] },
        },
      ],
    };
    assert.equal(
      (await provider.analyze(Buffer.from("video"), 3, "hello")).length,
      1,
    );
    assert.equal(sdk.calls.at(-1).model, "gemini-2.5-flash");
    assert.equal(
      sdk.calls.at(-1).contents[0].parts[0].inlineData.mimeType,
      "video/mp4",
    );
    assert.equal(sdk.calls.at(-1).config.responseMimeType, "application/json");
    response = { candidates: [{ finishReason: "MAX_TOKENS" }] };
    await assert.rejects(
      provider.analyze(Buffer.from("video"), 3, ""),
      /incomplete/,
    );
    response = {
      candidates: [
        {
          finishReason: "STOP",
          content: { parts: [{ text: "A door opens." }] },
        },
      ],
    };
    assert.equal(
      await provider.shorten("A woman slowly opens the wooden door.", 1.5),
      "A door opens.",
    );
    assert.equal(
      await provider.shorten("Door.", 1.5),
      "",
      "a longer revision cannot trigger another synthesis",
    );
    response = { candidates: [{ finishReason: "MAX_TOKENS" }] };
    await assert.rejects(
      provider.shorten("A woman opens a door.", 1),
      /incomplete/,
    );
    response = {
      audioContent: encodeWave(Buffer.alloc(48000)).toString("base64"),
    ...
    assert.equal(
      parseWave(await provider.synthesize("A door opens.")).duration,
      1,
    );
    ...
    await assert.rejects(
      provider.synthesize("A door opens."),
      (error) =>
        error instanceof Error &&
        !error.message.includes("sensitive-provider-content"),
    );
  } finally {
    globalThis.fetch = originalFetch;
    for (const [key, value] of Object.entries(previousEnv)) {
