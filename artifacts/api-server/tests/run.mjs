...
try {
  ...
  for (const name of [...modules, "routes", "fixtures", "safety.test"]) {
    const source = await readFile(
      new URL(
        name === "routes"
          ? "../src/routes/sema.ts"
          : ["safety.test", "fixtures"].includes(name)
            ? `./${name}.ts`
            : `../src/lib/${name}.ts`,
        import.meta.url,
      ),
    ...
    output = output
      ...
      .replace('from "google-auth-library"', 'from "./auth.mjs"');
    output = output.replace(
      /from "(?:@google\/genai|express|drizzle-orm|@workspace\/db|\.\/objectStorage.mjs|\.\/sema-demo.mjs|\.\/logger.mjs)"/g,
      'from "./fixtures.mjs"',
    );
    output = output.replace(
      ...
      (_all, names) =>
        names
          ...
          .map(
            (name) =>
              `const ${name.trim()} = { safeParse: data => ({ success: true, data }), parse: data => data };`,
          )
          .join("\n"),
    );
  ...
  await writeFile(
    join(directory, "auth.mjs"),
    'export class GoogleAuth { async getAccessToken() { return "test-access-token"; } }',
  );
  const result = spawnSync(
    process.execPath,
    ["--test", join(directory, "safety.test.mjs")],
    { stdio: "inherit" },
  );
