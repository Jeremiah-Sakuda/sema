...
export const semaVisualBeatsTable = pgTable("sema_visual_beats", {
  ...
  evidenceFrames: jsonb("evidence_frames").$type<string[]>().notNull().default([]),
...
export const semaDescriptionCandidatesTable = pgTable("sema_description_candidates", {
  ...
  windowId: text("window_id")
    .notNull()
    .references(() => semaNarrationWindowsTable.windowId, { onDelete: "cascade" }),
  ...
  languageFlags: jsonb("language_flags").$type<string[]>().notNull().default([]),
...
export const insertSemaProjectSchema = createInsertSchema(semaProjectsTable).omit({
  createdAt: true,
  updatedAt: true,
});
export const insertSemaVisualBeatSchema = createInsertSchema(semaVisualBeatsTable);
export const insertSemaNarrationWindowSchema = createInsertSchema(semaNarrationWindowsTable);
export const insertSemaDescriptionCandidateSchema = createInsertSchema(
  semaDescriptionCandidatesTable,
);
export const insertSemaReviewDecisionSchema = createInsertSchema(semaReviewDecisionsTable);
export const insertSemaProcessStageSchema = createInsertSchema(semaProcessStagesTable);
...
export type SemaDescriptionCandidate = typeof semaDescriptionCandidatesTable.$inferSelect;
